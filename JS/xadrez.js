...
window.addEventListener("resize", jogo);
window.addEventListener("load", jogo);

function jogo() {
    var container = document.getElementById("gameArea");
    
    // Definir largura e altura com base na janela
    var w = window.innerWidth * 0.8;
    var h = window.innerHeight * 0.8;
    
    // Calcular o tamanho do lado do quadrado para a grade 8x8
    var tsw = Math.min(w, h);
    var sw = (tsw - 16) / 8;  // Subtrai 16 para o padding/margem (se necessário)

    // Limpar o contêiner antes de adicionar novos quadrados
    container.innerHTML = '';
    
    // Estilizar o contêiner
    container.style.position = 'relative';
    container.style.width = tsw + 'px';
    container.style.height = tsw + 'px';
     // Centraliza o contêiner

    // Gerar e posicionar os quadrados
    for (var n = 0; n < 64; n++) {
        ...
        square.style.position = 'absolute';  // Garantir que o posicionamento seja absoluto

        // Calcular a posição de cada quadrado
        var row = Math.floor(n / 8);
        var col = n % 8;
        square.style.top = sw * row + 'px';
        square.style.left = sw * col + 'px';

        // Ajustar o tamanho da fonte
        square.style.fontSize = (sw * 3 / 4) + 'px';

        // Adicionar o quadrado ao contêiner
    ...
    function updateSquarecolor(){
        for(var n = 0; n < 64; n++){
            if(Math.floor(n/8)%2 === 0){
                if(n%2 === 0){
                    sqs[n].style.background = 'rgb(120, 69, 6)';
                }
                else {
                    sqs[n].style.background = 'rgb(234, 202, 119)';
                }
            }
            else {
                if(n%2 === 1){
